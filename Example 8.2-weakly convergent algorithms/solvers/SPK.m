...
function out2 = SPK(I,Bobs,P,center,para,stop)
...
if strcmp(stop.rule,'fixed')
    out2.SNR = zeros(1,stop.MAX);    out2.Time = zeros(1,stop.MAX);
end
time = cputime;
for iter = 1: stop.MAX
    m= norm(x1-x0,'fro')^2;
    ...
    if strcmp(stop.rule,'fixed')
        out2.SNR(iter) = 20*log10( In/norm(x1-I,'fro') );
        out2.Time(iter) = cputime - time;
        out2.image = x1;  out2.iter = iter;
    elseif strcmp(stop.rule,'EPS')
        out2.err = norm(x1-x,'fro')/norm(x,'fro');
        out2.SNR(iter) = 20*log10( In/norm(x1-I,'fro') );
        out2.Time(iter) = cputime - time;
        if out2.err <= stop.eps ||  iter >= stop.MAX
            out2.image = x1;   out2.iter = iter;
            return;
        end
    ...
    elseif strcmp(stop.rule,'SNR')
        ...
        out2.SNR(iter) = 20*log10( In/norm(x1-I,'fro') );
        out2.Time(iter) = cputime - time;
        if out2.SNR(end) >= stop.eps ||  iter >= stop.MAX
            out2.image = x1;   out2.iter = iter;
            return;
        end
    else
